...
void init()
{
    ...
    for(int i=0;i<19;i++)
    {
        printf("%s ", hash_name[i]);
    }

    char * word_hs[] = {"begin", "bool", "call", "char", "const", "do", "else", "end", "float",
                        "if", "odd", "procedure", "read", "repeat", "then", "until", "var", "while", "write"};

    for(int i=0; i<19; i++)
    {
        int inx;
        //printf("%d  \n", strlen(word_hs[i]));
        if(strlen(word_hs[i]) == 2)
        {
            inx = (word_hs[i][0]-'a'+word_hs[i][1]-'a')%19;
            //printf("%d ", word_hs[i][0]-'a'+word_hs[i][1]-'a');
        }
        else
            inx = (word_hs[i][0]-'a'+word_hs[i][1]-'a'+word_hs[i][2]-'a')%19;


        while(1){
            if(hash_name[inx]==0)
            {
                hash_name[inx]=word_hs[i];
                break;
            }
            inx++;
            if(inx==19) inx=0;
...
int getsym()
{
    ...
    if(ch>='a'&&ch<='z')
    {
        ...
        /*int inx, flag=0;
        if(strlen(id) == 2)
            inx = (id[0]-'a'+id[1]-'a')%19;
        else
            inx = (id[0]-'a'+id[1]-'a'+id[2]-'a')%19;
        while(1){
            if(!strcmp(hash_name[inx], id))
            {
                //printf("找到");
                flag = 1;
                if(strcmp(hash_name[inx], "begin")) sym=beginsym;
                else if(strcmp(hash_name[inx], "bool")) sym=boolsym;
                else if(strcmp(hash_name[inx], "call")) sym=callsym;
                else if(strcmp(hash_name[inx], "char")) sym=charsym;
                else if(strcmp(hash_name[inx], "const")) sym=constsym;
                else if(strcmp(hash_name[inx], "do")) sym=dosym;
                else if(strcmp(hash_name[inx], "else")) sym=elsesym;
                else if(strcmp(hash_name[inx], "end")) sym=endsym;
                else if(strcmp(hash_name[inx], "float")) sym=floatsym;
                else if(strcmp(hash_name[inx], "if")) sym=ifsym;
                else if(strcmp(hash_name[inx], "odd")) sym=oddsym;
                else if(strcmp(hash_name[inx], "procedure")) sym=procsym;
                else if(strcmp(hash_name[inx], "read")) sym=readsym;
                else if(strcmp(hash_name[inx], "repeat")) sym=repeatsym;
                else if(strcmp(hash_name[inx], "then")) sym=thensym;
                else if(strcmp(hash_name[inx], "until")) sym=untilsym;
                else if(strcmp(hash_name[inx], "var")) sym=varsym;
                else if(strcmp(hash_name[inx], "while")) sym=whilesym;
                else if(strcmp(hash_name[inx], "write")) sym=writesym;
                break;
            }
            inx++;
            if(inx==19) inx=0;
...
int block(int lev,int tx,bool* fsys)
{
    ...
    //for(int i=0; i<tree_space; i++) printf("\t");tree_space++;
    //printf("block\n");
    //printf("    ");
